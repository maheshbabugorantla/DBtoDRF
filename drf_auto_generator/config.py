...
from pydantic import (
    BaseModel, Field, validator, ValidationError, root_validator, field_validator, model_validator
)
...
class DatabaseSettings(BaseModel):
    ...
    @field_validator('PORT', mode='before') # Use pre=True to handle string conversion early
    def validate_port(cls, v):
        ...
        if isinstance(v, int):
            ...
            try:
             ...
             raise TypeError(f"Port must be an integer or string containing only digits, got {type(v).__name__}")
        ...
        if isinstance(v, list):
            for item in v:
                if not isinstance(item, str):
                    raise TypeError(f"Table names in include/exclude lists must be strings, found: {type(item).__name__}")
                if not item.strip():
                    raise ValueError("Table names in include/exclude lists cannot be empty or just whitespace.")
...
def _validate_and_parse_config(config_dict: Dict[str, Any]) -> ToolConfigSchema:
    ...
    try:
        ...
        for error in e.errors():
            ...
            if 'value_error.identifier' in str(err_type) or ('value_error' in str(err_type) and any(x in loc_parts for x in ['project_name', 'app_name'])):
                 print(f"    Hint: Ensure the value is a valid Python variable name (letters, numbers, underscores, not starting with a number, not a keyword like 'class' or 'list').", file=sys.stderr)
            elif 'value_error.literal' in str(err_type) and 'relation_style' in loc_parts:
                 allowed = error.get('ctx', {}).get('expected', "'pk', 'link', 'nested'")
                 print(f"    Hint: Allowed values are: {allowed}", file=sys.stderr)
            elif 'type_error.str' in str(err_type) and any(x in loc_parts for x in ['include_tables', 'exclude_tables']):
                 print(f"    Hint: Items in this list should be strings (text).", file=sys.stderr)
            elif 'value_error.str.not_empty' in str(err_type) and any(x in loc_parts for x in ['include_tables', 'exclude_tables']):
                 print(f"    Hint: Table names cannot be empty strings.", file=sys.stderr)
            elif 'value_error.port' in str(err_type): # Custom error type hint might be needed
                 print(f"    Hint: Port must be a number between 0 and 65535.", file=sys.stderr)
...
def load_config(config_path: Optional[str], cli_args: argparse.Namespace) -> ToolConfigSchema:
    """
            ...
            if config_file.is_file():
                with open(config_file, 'r', encoding='utf-8') as f:
                    ...
                    if yaml_config and isinstance(yaml_config, dict):
                        ...
                        logger.warning(f"Content in config file {config_path} is not a dictionary. Ignoring file content.")
            else:
                logger.warning(f"Config file not found at {config_path}. Using defaults and CLI arguments.")
    ...
    for key, value in cli_dict.items():
        ...
        if value is not None and key != 'databases' and hasattr(ToolConfigSchema, key): # Check if it's a valid config key
