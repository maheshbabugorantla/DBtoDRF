...
def generate_settings_code(project_name: str, app_name: str, kwargs: Dict[str, Any]) -> str:
    ...
    # Use PageNumberPagination to match OpenAPI spec format (count, next, previous, results)
    rest_framework_dict = add_location(ast.Dict(
        keys=[
            ...
            create_string_constant('EXCEPTION_HANDLER')
        ],
        values=[
            ...
            create_string_constant('rest_framework.pagination.PageNumberPagination'),
            add_location(ast.Constant(value=10)),
            create_string_constant(f'{app_name}.exception_handlers.custom_exception_handler')
        ]
    ))
    body.append(
        create_assign(
...
def generate_exception_handler_code() -> str:
    """
    Generate custom exception handler that matches the OpenAPI ErrorDetail schema.

    The OpenAPI spec defines error responses as:
    {
        "detail": "string",
        "code": "string",
        "errors": { ... }
    ...
    This handler formats DRF exceptions to match that schema.
    """
    code = '''"""
Custom exception handler that formats errors to match the OpenAPI ErrorDetail schema.
...
def custom_exception_handler(exc, context):
    ...
    This matches the ErrorDetail schema defined in the OpenAPI specification.
    """
    ...
    if response is not None:
        ...
        # Include field-level errors if present (for validation errors)
        if hasattr(exc, 'detail'):
            if isinstance(exc.detail, dict):
                error_response['errors'] = exc.detail
            elif isinstance(exc.detail, list):
                error_response['errors'] = {'non_field_errors': exc.detail}
