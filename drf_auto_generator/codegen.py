...
from jinja2 import Environment, FileSystemLoader, select_autoescape, ext as jinja2_extensions
...
try:
    import black
    from black import FileMode, format_str as black_format_str, NothingChanged as BlackNothingChanged
    ...
    logging.warning("Package 'black' not found. Generated Python code will not be auto-formatted.")
...
def setup_jinja_env() -> Environment:
    ...
    env = Environment(
        ...
        autoescape=select_autoescape(['html', 'xml', 'j2']), # Enable autoescaping for safety
...
def format_python_code_using_black(filepath: Path, code_string: str) -> str:
    ...
    try:
        ...
        logger.error(f"Could not format Python code using Black: {e}", exc_info=False) # Keep log concise
        logger.warning("Writing unformatted Python code due to Black error.")
        return code_string # Return the original string on error


def generate_file_from_template(env: Environment, template_name: str, context: Dict[str, Any], output_path: Path):
    ...
    try:
        ...
        if output_path.suffix == '.py':
            ...
            final_content = format_python_code_using_black(output_path, rendered_content)
        else:
        ...
        logger.error(f"Error generating file from template '{template_name}' to '{output_path}': {e}", exc_info=True)
        # Decide if we should raise the exception or just log it
        raise e


def setup_project_structure(output_dir: str, project_name: str, app_name: str, env: Environment, config: Dict[str, Any]):
    ...
    try:
        ...
        os.chmod(manage_py_path, current_st.st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
    except Exception as e:
        logger.warning(f"Could not make manage.py executable: {e}")

    # --- Generate project-level files ---
    generate_file_from_template(env, '__init__.py.j2', context, project_path / '__init__.py')
    generate_file_from_template(env, 'settings.py.j2', context, project_path / 'settings.py')
    generate_file_from_template(env, 'root_urls.py.j2', context, project_path / 'urls.py')
    ...
    generate_file_from_template(env, '__init__.py.j2', context, app_path / '__init__.py')
    generate_file_from_template(env, 'apps.py.j2', context, app_path / 'apps.py')
    generate_file_from_template(env, '__init__.py.j2', context, app_path / 'migrations' / '__init__.py')
...
def generate_django_code(ir_list: List[TableInfo], config: Dict[str, Any]):
    ...
    logger.debug(f"Context check for admin.py - tables type: {type(app_context.get('tables'))}")
    if isinstance(app_context.get('tables'), list):
        logger.debug(f"Context check for admin.py - tables length: {len(app_context['tables'])}")
        if app_context['tables']: # Check if list is not empty
            logger.debug(f"Context check for admin.py - first table model name: {app_context['tables'][0].model_name}")
        else:
            logger.debug("Context check for admin.py - 'tables' list is empty.")
    else:
        logger.debug("Context check for admin.py - 'tables' key not found or not a list.")
    # --- End DEBUG block ---
    ...
    generate_file_from_template(env, 'models.py.j2', app_context, app_path / 'models.py')

    # 5. Generate serializers.py
    logger.info("Generating serializers.py...")
    generate_file_from_template(env, 'serializers.py.j2', app_context, app_path / 'serializers.py')
    ...
    generate_file_from_template(env, 'requirements.txt.j2', req_context, Path(output_dir) / 'requirements.txt')

    logger.info(f"Django project '{project_name}' generated successfully in '{output_dir}'")
    logger.info("--------------------------------------------------")
    logger.info("Next Steps:")
    logger.info(f"1. cd \"{output_dir}\"")
    logger.info("2. Create and activate a virtual environment (e.g., python -m venv venv && source venv/bin/activate)")
    logger.info("3. Install requirements: pip install -r requirements.txt")
    logger.info(f"4. Configure DATABASES in '{project_name}/settings.py' (passwords, etc.)")
    logger.info(f"5. Review/Configure production settings in '{project_name}/settings.py':")
    ...
    logger.info("     - Consider uncommenting SECURE_* settings, STATICFILES_STORAGE, GZipMiddleware.")
    ...
    logger.info("9. Access the API docs at: http://<your_host>:8000/api/schema/swagger-ui/")
    logger.info("--------------------------------------------------")
