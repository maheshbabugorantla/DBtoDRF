...
def generate_openapi_schema_object(table: TableInfo, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for field_info in table.fields:
        ...
        if not field_info['is_handled_by_relation'] or field_info['is_pk']:
            ...
            openapi_schema = field_info.get('openapi_schema', {'type': 'string'}) # Get stored schema
            ...
            original_col = next((c for c in table.columns if c.name == field_info['original_column_name']), None)
            if not openapi_schema.get('nullable', True) and not openapi_schema.get('readOnly', False) and (not original_col or original_col.default is None):
            ...
            if relation_style == 'pk':
                ...
                properties[rel_name] = {
                 ...
                 properties[rel_name] = {
                     ...
                     'nullable': rel_info['django_field_options'].get('null', True) # Reflect nullability
                 }
        elif rel_type in ('one-to-many', 'many-to-many'):
            # These are represented as arrays of related items
            item_schema = {}
            if relation_style == 'pk':
                item_schema = {'type': 'integer'} # Assuming integer PKs for related items
...
def generate_openapi_input_schema(table: TableInfo, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for name, prop_schema in output_schema.get('properties', {}).items():
        if not prop_schema.get('readOnly', False):
            ...
            if original_field:
                original_col = next((c for c in table.columns if c.name == original_field['original_column_name']), None)
                if original_col and not original_col.nullable and original_col.default is None:
    ...
    for rel_info in table.relationships:
         ...
         if rel_info['type'] == 'many-to-one':
             fk_field_name = clean_field_name(rel_info['source_columns'][0]) # Original FK field name
             ...
             if relation_style == 'pk':
                 ...
                 input_properties.pop(rel_name, None) # Remove nested object ref from input
                 if not is_fk_nullable and fk_field_name in input_properties:
                      if fk_field_name not in input_required:
                        input_required.append(fk_field_name)
...
def generate_paths_for_table(table: TableInfo, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not pk_field_info:
        logger.warning(f"Table {table.name} has no primary key field identified in 'fields'. Skipping CRUD path generation.")
        return {}

    pk_name = pk_field_info['name']
    pk_schema = pk_field_info.get('openapi_schema', {'type': 'integer'}) # Get PK schema
    ...
    patch_schema_ref = f'#/components/schemas/{model_name}PatchInput' # Separate schema for PATCH
    ...
    paths[list_create_path] = {
        ...
        'get': {
            ...
            'parameters': [
                ...
                {'name': 'page_size', 'in': 'query', 'required': False, 'schema': {'type': 'integer'}, 'description': 'Number of results per page'},
            ...
            'responses': {
                        ...
                        'application/json': {
                            ...
                            'schema': {
                                ...
                                'properties': {
                                    ...
                                    'next': {'type': 'string', 'format': 'uri', 'nullable': True, 'description': 'URL to the next page.'},
                                    'previous': {'type': 'string', 'format': 'uri', 'nullable': True, 'description': 'URL to the previous page.'},
                                    'results': {
                                        'type': 'array',
                ...
                'default': {'$ref': '#/components/responses/Error'} # Generic error response
            }
        },
        # --- POST (Create) ---
        'post': {
            ...
            'requestBody': {
                ...
                'content': {
                    ...
                    'multipart/form-data': {'schema': {'$ref': input_schema_ref}}, # If file uploads possible
                }
            },
            'responses': {
                '201': {
    ...
    paths[detail_path] = {
        ...
        'delete': {
            ...
            'responses': {
                 '204': {'description': f"{model_name} deleted successfully."}, # No content
                 '404': {'$ref': '#/components/responses/NotFound'},
                 'default': {'$ref': '#/components/responses/Error'}
            }
        }
...
def generate_openapi_spec(ir_list: List[TableInfo], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    error_schema_detail = {
        "ErrorDetail": {
            "type": "object",
            "properties": {
                "detail": { "type": "string", "description": "A human-readable error message." },
    ...
    common_responses = {
        ...
        "InvalidInput": {
            ...
            "content": { "application/json": { "schema": { "$ref": "#/components/schemas/ErrorDetail"}}} # Use ErrorDetail or a more specific validation error schema
        },
        "Error": {
    ...
    spec = {
        ...
        'tags': [{'name': tag, 'description': f'Operations related to {tag}s'} for tag in sorted(list(all_tags))],
        'paths': all_paths,
        'components': {
            'schemas': {**schemas, **error_schema_detail}, # Combine model schemas and error schema
...
def save_openapi_spec(spec_dict: Dict[str, Any], output_dir: str, filename: str = "openapi.yaml"):
